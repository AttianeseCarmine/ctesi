...
def evaluate_mae_rmse(model, loader, device, rank=0):
    ...
    for batch in loader:
        ...
        if points is not None:
            ...
            # meglio SKIPPARE invece di usare gt=pred (che falserebbe rmse/mae)
            continue
...
# ----------------------------------------------------------
    # 1. Definizione Optimizer e Scaler (COME PRIMA)
    # ----------------------------------------------------------
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scaler = GradScaler() if args.amp else None

    # ----------------------------------------------------------
    # 2. Setup Directory Output (SPOSTATO SOPRA PER IL RESUME)
    # ----------------------------------------------------------
    # Definiamo subito dove sono i checkpoint per poterli cercare
    # ----------------------------------------------------------
    # 2. Setup Directory Output
    # ----------------------------------------------------------
    if args.out is None:
         config_name = f"{args.model}_{args.dataset}"
    ...
    # Inizializza i valori di default PRIMA del controllo resume
    start_epoch = 1
    best_mae = float("inf")
    best_val_scores = {"mae": [float("inf")], "rmse": [float("inf")], "epoch": [0]} # <--- SPOSTATO QUI
    hist_val_scores = {}
    loss_info = {}
    ...
    if os.path.exists(resume_path):
        ...
        if args.amp and "grad_scaler_state_dict" in checkpoint and checkpoint["grad_scaler_state_dict"] is not None:
            scaler.load_state_dict(checkpoint["grad_scaler_state_dict"])
            
        start_epoch = checkpoint["epoch"] + 1
        
        # Recupera sia best_mae che il dizionario completo best_val_scores
        if "best_val_scores" in checkpoint and checkpoint["best_val_scores"] is not None:
            best_val_scores = checkpoint["best_val_scores"] # <--- AGGIORNAMENTO IMPORTANTE
            best_mae = best_val_scores["mae"][0]
    ...
    for epoch in range(start_epoch, args.total_epochs + 1):
        ...
        if local_rank == 0:
            ...
            if not eval_now:
                ...
                hist_val_scores.setdefault("rmse", [])
                hist_val_scores["rmse"].append((epoch, float(val_rmse)))
                ...
                if is_best:
                    ...
                    best_val_scores["rmse"][0] = float(val_rmse)  # rmse associato al best mae
