...
def main(config_path, checkpoint_path):
    ...
    print(f"="*60)
    print(f"👁️  VISUALIZE STAGE 2 (EBC-Head)")
    ...
    if os.path.exists(checkpoint_path):
        ...
        print("    Assicurati di aver finito il training o passa il path corretto con --ckpt")
    ...
    print(f"🖼️  Estraggo 4 immagini casuali dal val set ({len(val_set)} totali)...")
    ...
    with torch.no_grad():
        for i, idx in enumerate(indices):
            ...
            # Forward completo (Pi + EBC)
            # Nello stage 2 ci interessa vedere se EBC conta giusto sui blocchi selezionati da Pi
            outputs = model(img_tensor)
            
            pred_density = outputs['density_map']   # Mappa densità finale
            pred_count = outputs['pred_count'].item()
            gt_count = gt_density.sum().item()
            
            # Immagine originale per display
            img_np = denormalize(sample['image'])
            
            # Colonna 1: Input Image
            axes[i, 0].imshow(img_np)
            axes[i, 0].set_title(f"Image {idx}", fontsize=10)
            axes[i, 0].axis('off')
            
            # Colonna 2: Ground Truth Density
            axes[i, 1].imshow(gt_density.squeeze().cpu().numpy(), cmap='jet')
            axes[i, 1].set_title(f"GT Density (Count: {gt_count:.1f})", fontsize=10)
            axes[i, 1].axis('off')
            
            # Colonna 3: Predicted Density
            pred_map = pred_density.squeeze().cpu().numpy()
            axes[i, 2].imshow(pred_map, cmap='jet')
            
            # Titolo con errore
            err = abs(pred_count - gt_count)
            color = "green" if err < 0.1 * gt_count else "red"
            axes[i, 2].set_title(f"Pred: {pred_count:.1f} | Diff: {err:.1f}", color=color, fontsize=12, fontweight='bold')
            axes[i, 2].axis('off')

    out_path = "check_stage2.png"
    plt.savefig(out_path, bbox_inches='tight')
    print(f"\n✅ Grafico salvato in: {out_path}")
    print("   Apri l'immagine per vedere i risultati!")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="config_sha.yaml")
    # Default: cerca il best model dello stage 2
    parser.add_argument("--ckpt", default="experiments/sha_zip_clip_ebc/stage2/best_stage2_model.pth")
