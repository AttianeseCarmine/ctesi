...
class JointLoss(nn.Module):
    def __init__(self, clip_loss_fn, lambda_zip=1.0, lambda_clip=1.0, lambda_count=10.0):
        super().__init__()
        # Loss per Stage 1 (Filtro) - BCE con pos_weight per sbilanciamento classi
        self.zip_loss = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([15.0]).cuda())
        
        # Loss per Stage 2 (CLIP-EBC) - Passata da fuori
        self.clip_loss = clip_loss_fn
        
        # Loss Finale (Consistenza Conteggio)
        # Usiamo L1Loss sulla mappa di densità
    ...
    def forward(self, outputs, targets):
        ...
        # Confronta i logits di ZIP con la maschera binaria GT
        l_zip = self.zip_loss(outputs['pi_logits'], targets['mask'])
        
        # 2. Loss CLIP (Conteggio Semantico + OT/TV)
        # Passiamo density e points perché la nuova CLIPEBCLoss li richiede
        ...
        if pred_density.shape[-2:] != gt_density.shape[-2:]:
            ...
            # Calcoliamo il fattore di scala dell'area (es. 16*16 = 256)
            scale_factor = (h_in * w_in) / (h_out * w_out)
            
            # Usiamo Adaptive Avg Pool per downsampling corretto (conserva la densità media)
            # Moltiplichiamo per scale_factor per conservare il CONTEGGIO TOTALE (Somma)
            gt_density_resized = F.adaptive_avg_pool2d(gt_density, (h_out, w_out)) * scale_factor
        else:
            gt_density_resized = gt_density

        # Ora le dimensioni matchano (es. 28x28 vs 28x28) -> Nessun errore
        ...
        # Aggiungiamo i dettagli di CLIP (es. l_ot, l_tv) se ci sono
