...
def resume_if_exists(model, optimizer, exp_dir, device, filename="last.pth"):
    """
    Tenta di caricare il checkpoint per riprendere il training.
    Ora accetta un filename personalizzato (default: last.pth).
    """
    last_ck = os.path.join(exp_dir, filename)
    if os.path.isfile(last_ck):
        # Fix per PyTorch > 2.6
        ckpt = torch.load(last_ck, map_location=device, weights_only=False)
...
def save_checkpoint(model, optimizer, epoch, val_metric, best_metric, exp_dir, is_best=False, best_name="best_model.pth", last_name="last.pth"):
    """
    Salva il checkpoint.
    Ora accetta nomi personalizzati per i file best e last.
    """
    os.makedirs(exp_dir, exist_ok=True)
    ckpt = {
        "epoch": epoch,
    ...
    # Salva sempre il 'last' (checkpoint completo per resume)
    torch.save(ckpt, os.path.join(exp_dir, last_name))
    
    if is_best:
        # Salva il 'best' (solo pesi modello)
