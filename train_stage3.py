...
def main(config_path: str):
    ...
    # 2. Caricamento Pesi: PARTIAMO DA STAGE 1 (perché Stage 2 era debole)
    stage1_dir = os.path.join(config["EXP"]["OUT_DIR"], run_name, "stage1")
    stage1_ckpt = os.path.join(stage1_dir, "best_stage1_model.pth")
    
    if not os.path.exists(stage1_ckpt):
        stage1_ckpt = os.path.join(stage1_dir, "last_stage1_model.pth")
        
    if os.path.exists(stage1_ckpt):
        print(f"📥 Reset: Carico pesi STAGE 1 (Maschera Ottima) da: {stage1_ckpt}")
        ckpt = torch.load(stage1_ckpt, map_location=device, weights_only=False)
        # Se nel checkpoint c'è 'model', usa quello, altrimenti usa il dict intero
        state_dict = ckpt.get('model', ckpt)
        model.load_state_dict(state_dict, strict=False)
    else:
        print("⚠️ ATTENZIONE: Nessun checkpoint trovato! Parto da zero.")
